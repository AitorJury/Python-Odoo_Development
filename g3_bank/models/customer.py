...
class Customer(models.Model):
    ...
    _inherit = 'res.users'
        #FIXME Solucionar dominio, quitándolo si es necesario, para permitir asociar cuentas con clientes
...
#   Se añade funcionalidad, no permite borrar un customer que tenga cuentas asociadas.
    def unlink(self):
        for record in self:
            # Buscamos si este usuario está en alguna cuenta de nuestro modelo
            accounts = self.env['g3_bank.account'].sudo().search([('customer_ids', 'in', record.id)])
            if accounts:
